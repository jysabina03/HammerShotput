...
class Score_dee:
    def __init__(self, turn, dis, ball_dx):
        ...
        self.font_s = load_font('ENCR10B.TTF', 10)
    ...
    def draw(self):
        ...
        if self.type == 0:
            self.image.clip_composite_draw(self.type * 70, 4 * 70, 70, 70, 0, '', 50 * (self.turn+1) / 2+10, 450+57, 70*1.2,
                                           70*1.2)
            self.font_s.draw(50 * (self.turn+1) / 2 - 15+10, 450+55 + 15, f'{self.distance}m', (14, 14, 14))
        else:
            self.image.clip_composite_draw(self.type * 70, 4 * 70, 70, 70, 0, '', 50 * self.turn / 2+10, 350+57,
                                           70*1.2, 70*1.2)
            self.font_s.draw(50 * self.turn / 2 - 15+10, 350+55 + 15, f'{self.distance}m', (14, 14, 14))
